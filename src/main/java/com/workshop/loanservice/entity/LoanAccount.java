...
/**
 * Modern loan account entity mapped to the normalized 'loan_accounts' table.
 * Uses proper FK relationships instead of denormalized borrower fields.
 * Financial amounts use BigDecimal; dates use LocalDate.
 */
@Entity
...
public class LoanAccount {
    ...
    // Normalized FK relationship — replaces the denormalized borrower fields in legacy
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "borrower_id", nullable = false)
    private Borrower borrower;

    // FK to loan product — replaces the product code string lookup
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    // Bidirectional relationship to payments
